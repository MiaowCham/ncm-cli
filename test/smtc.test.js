...
test('MediaPlayer 模式复用 bridge 并等待命令 ACK 与播放器事件', async () => {
  const child = new FakeChild();
  const events = [];
  const bridge = await createSmtcBridge({
    platform: 'win32',
    ...
    spawnImpl: spawning(child),
    helperCommand: 'mock.exe',
    onPlayerEvent: (event, generation) => events.push({ event: event.event, generation }),
    commandTimeoutMs: 100,
    closeTimeoutMs: 5
  });
  assert.equal(child.message(0).mode, 'media-player');
  const loading = bridge.load('https://example.test/song.mp3', {
    positionMs: 1250, volume: 80, durationMs: 9000,
    metadata: { name: '测试歌曲', artists: ['测试歌手'] }
  });
  await new Promise((resolve) => setImmediate(resolve));
  const load = child.message(child.stdin.writes.length - 1);
  assert.deepEqual(
    { type: load.type, url: load.url, positionMs: load.positionMs, volume: load.volume },
    { type: 'load', url: 'https://example.test/song.mp3', positionMs: 1250, volume: 80 }
  );
  child.stdout.emit('data', Buffer.from(`${JSON.stringify({
    v: 1, sessionId: bridge.sessionId, type: 'ack', command: 'load', commandId: load.commandId
  })}\n`));
  assert.equal(await loading, 1);
  child.stdout.emit('data', Buffer.from(`${JSON.stringify({
    v: 1, sessionId: bridge.sessionId, type: 'player_event', event: 'ended'
  })}\n`));
  assert.deepEqual(events, [{ event: 'ended', generation: 1 }]);
  await bridge.close();
});
