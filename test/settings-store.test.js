...
import {
  ...
  DEFAULT_IMAGE_PROTOCOL, DEFAULT_PLAYER_BACKEND, DEFAULT_SEARCH_LIMIT
...
test('音质设置使用独立文件并可持久化', async () => {
  ...
  try {
    ...
    await saveSettings({
      quality: 'lossless', playerBackend: 'vlc', imageProtocol: 'sixel', lyricOffsetMs: 1500, smtcOffsetMs: 250, searchLimit: 40,
...
test('缺失或非法设置回退默认值，保存时拒绝非法值', async () => {
  ...
  try {
    assert.deepEqual(await loadSettings(file), {
      quality: 'standard', playerBackend: DEFAULT_PLAYER_BACKEND, imageProtocol: DEFAULT_IMAGE_PROTOCOL, lyricOffsetMs: DEFAULT_LYRIC_OFFSET_MS,
...
test('旧配置迁移默认偏移，部分保存不会覆盖另一项设置', async () => {
  ...
  try {
    ...
    assert.deepEqual(await loadSettings(file), {
      quality: 'lossless', playerBackend: DEFAULT_PLAYER_BACKEND, imageProtocol: DEFAULT_IMAGE_PROTOCOL, lyricOffsetMs: DEFAULT_LYRIC_OFFSET_MS,
      smtcOffsetMs: DEFAULT_SMTC_OFFSET_MS, searchLimit: DEFAULT_SEARCH_LIMIT, apiBaseUrl: null
    });
    await saveSettings({ lyricOffsetMs: -500 }, file);
    assert.deepEqual(await loadSettings(file), {
      quality: 'lossless', playerBackend: DEFAULT_PLAYER_BACKEND, imageProtocol: DEFAULT_IMAGE_PROTOCOL, lyricOffsetMs: -500, smtcOffsetMs: DEFAULT_SMTC_OFFSET_MS,
    ...
    assert.deepEqual(await loadSettings(file), {
      quality: 'higher', playerBackend: DEFAULT_PLAYER_BACKEND, imageProtocol: DEFAULT_IMAGE_PROTOCOL, lyricOffsetMs: -500,
