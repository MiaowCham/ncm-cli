...
已登录用户可用 `/lspl` 列出自己的歌单，“喜欢的音乐”会置顶显示；输入序号可进入歌单详情。也可用 `/pl <id>` 直接预览指定歌单。歌单详情显示创建者、歌曲数、播放量、描述及前 15 首歌曲，并提供以下操作：

```text
p                       从第一首开始播放歌单
e > playlist.txt        导出完整歌曲列表（也支持 e | playlist.txt）
u                       打印网易云歌单链接
q                       返回上一级
```

导出不会覆盖已有文件。歌单播放会自动播放下一首，并跳过当前账号无法取得播放链接的歌曲。
...
Windows 播放时会向 SMTC 发布标题、艺术家、专辑、封面、播放状态和应用偏移后的时间线，并接受系统的播放、暂停、停止、快进、后退与进度条跳转控制。歌单播放还会按当前位置启用系统上一首/下一首按钮，单曲播放保持禁用。helper 缺失或初始化失败时会自动降级，不影响终端播放控制。
