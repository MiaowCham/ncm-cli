...
已登录用户可用 `/lspl` 列出自己的歌单，“喜欢的音乐”会置顶显示。在交互式终端中可用 ↑/↓ 或滚轮滚动选择，按 Enter 进入歌单详情，按 q 或 Esc 返回；从详情返回后会保留原选择位置。非交互式终端仍可输入序号选择。也可用 `/pl <id>` 直接预览指定歌单。歌单详情显示创建者、歌曲数、播放量、描述及前 15 首歌曲，并提供以下操作：
